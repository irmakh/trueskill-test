...
# from Table  of the Herbrich et al. paper.
...
class TruncateFactor(Factor):
  ...
  def __init__(self, variable, V, W, epsilon,winconstant):
    super(TruncateFactor, self).__init__([variable])
    self.var = variable
    self.V = V
    self.W = W
    self.epsilon = epsilon
    self.winconstant = winconstant
    print(self.winconstant)
  ...
  def Update(self):
    ...
    V = self.V(*args)*self.winconstant #TODO this changes 2-1
    W = self.W(*args)*self.winconstant
...
def AdjustPlayers(players,winconstant = 1):
  """
